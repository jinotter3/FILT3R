...
class ARCroco3DStereoConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        kalman_ema_beta_info=0.05,
        kalman_ema_info_floor=1e-2,
        kalman_ema_delta_floor=1e-2,
        kalman_ema_nis_eta=5e-3,
        kalman_r_scale=None,
        kalman_fixed_r=1.0,
        kalman_soft_reset_base=0.85,
        kalman_soft_reset_min=0.70,
        kalman_soft_reset_max=0.95,
        kalman_soft_reset_adaptive_scale=0.10,
        kalman_newqr_beta_info=0.05,
        kalman_newqr_rel_mode="entropy",
        kalman_newqr_rel_floor=0.05,
        kalman_newqr_rel_ceil=0.95,
        kalman_newqr_r_scale=None,
        kalman_newqr_use_nis=True,
        kalman_newqr_nis_thr=10.0,
        kalman_newqr_nis_alpha=0.3,
        **croco_kwargs,
    ):
        super().__init__()
        # Keep loading old checkpoints/config strings that still pass removed update-rule args.
        croco_kwargs.pop("ttsa3r_tau", None)
        croco_kwargs.pop("soc3r_p_init", None)
        croco_kwargs.pop("soc3r_q_scale", None)
        croco_kwargs.pop("soc3r_r_scale", None)
        ...
        self.ema_base_beta = ema_base_beta
        self.kalman_p_init = kalman_p_init
        self.kalman_gamma_p = kalman_gamma_p
        self.kalman_q_min = kalman_q_min
        self.kalman_q_max = kalman_q_max
        self.kalman_alpha_q = kalman_alpha_q
        self.kalman_tau_q = kalman_tau_q
        self.kalman_r_min = kalman_r_min
        self.kalman_c_r = kalman_c_r
        self.kalman_k_min = kalman_k_min
        self.kalman_k_max = kalman_k_max
        self.kalman_ema_beta_delta = kalman_ema_beta_delta
        self.kalman_ema_beta_info = kalman_ema_beta_info
        self.kalman_ema_info_floor = kalman_ema_info_floor
        self.kalman_ema_delta_floor = kalman_ema_delta_floor
        self.kalman_ema_nis_eta = kalman_ema_nis_eta
        self.kalman_r_scale = kalman_r_scale
        self.kalman_fixed_r = kalman_fixed_r
        self.kalman_soft_reset_base = kalman_soft_reset_base
        self.kalman_soft_reset_min = kalman_soft_reset_min
        self.kalman_soft_reset_max = kalman_soft_reset_max
        self.kalman_soft_reset_adaptive_scale = kalman_soft_reset_adaptive_scale
        self.kalman_newqr_beta_info = kalman_newqr_beta_info
        self.kalman_newqr_rel_mode = kalman_newqr_rel_mode
        self.kalman_newqr_rel_floor = kalman_newqr_rel_floor
        self.kalman_newqr_rel_ceil = kalman_newqr_rel_ceil
        self.kalman_newqr_r_scale = kalman_newqr_r_scale
        self.kalman_newqr_use_nis = kalman_newqr_use_nis
        self.kalman_newqr_nis_thr = kalman_newqr_nis_thr
        self.kalman_newqr_nis_alpha = kalman_newqr_nis_alpha
...
class ARCroco3DStereo(CroCoNet):
    ...
    def __init__(self, config: ARCroco3DStereoConfig):
        ...
        # Keep the full AR config for update-policy hparams.
        # CroCoNet.__init__ calls PreTrainedModel.__init__ and overwrites self.config.
        self._arcroco_config = config
    ...
    @staticmethod
    def _lookup_hparam(source, name):
        if source is None:
            return None, False
        ...
        if hasattr(source, name):
            return getattr(source, name), True
        return None, False

    def _get_hparam(self, name, default=None):
        value, found = self._lookup_hparam(getattr(self, "hparams", None), name)
        if found:
            return value
        ...
        value, found = self._lookup_hparam(getattr(self, "config", None), name)
        if found:
            return value

        value, found = self._lookup_hparam(getattr(self, "_arcroco_config", None), name)
        if found:
            return value
    ...
    def _resolve_model_update_type(self):
        update_type = self._get_hparam("model_update_type", "cut3r")
        legacy_aliases = {
            "kalman-ema-fixedr": "filt3r",
        }
        update_type = legacy_aliases.get(update_type, update_type)
        if update_type not in {"cut3r", "ttt3r", "filt3r"}:
            raise ValueError(
                f"Unsupported model_update_type='{update_type}'. "
                "Public release supports only: cut3r, ttt3r, filt3r."
            )
        return update_type

    def _requires_attn_for_update(self):
        return self._resolve_model_update_type() == "ttt3r"

    def _ema_update(self, ema_prev, x, beta, do_update):
        """
        ema_prev: [B] or None
        x: [B]
        do_update: [B] bool
        ...
        if ema_prev is None:
            return x
        return torch.where(do_update, (1.0 - beta) * ema_prev + beta * x, ema_prev)

    def _apply_stream_reset(
        self,
        state_feat,
        mem,
        init_state_feat,
        ...
        if reset_mask is None:
            return state_feat, mem, state_cov, False

        if reset_mask.dtype == torch.bool:
            reset_flag = reset_mask
        else:
            reset_flag = reset_mask > 0.5
        if reset_flag.dim() > 1:
            reset_flag = reset_flag.view(reset_flag.shape[0], -1).any(dim=1)
        hard_reset_now = bool(reset_flag.any().item())
        if not hard_reset_now:
            return state_feat, mem, state_cov, False

        reset_flag_f = reset_flag.to(device=state_feat.device, dtype=state_feat.dtype)
        blend_state = reset_flag_f[:, None, None]
        state_feat = init_state_feat * blend_state + state_feat * (1.0 - blend_state)
        mem = init_mem * blend_state + mem * (1.0 - blend_state)
        return state_feat, mem, state_cov, True

    def _compute_kalman_ema_gain_and_cov(
        self,
        update_mask,
        state_cov,
        state_feat,
        new_state_feat,
        prev_candidate_state_feat,
        cross_attn_state,
        feat_i,
        prev_feat_i,
        kalman_stats,
        use_nis_correction=False,
        use_ttt3r_r=False,
        fixed_r_t=None,
        kalman_diag_stats=None,
    ):
        eps = 1e-6
        del state_feat, cross_attn_state, feat_i, prev_feat_i, use_nis_correction
        del use_ttt3r_r, kalman_diag_stats
        batch_update = update_mask.squeeze(-1).squeeze(-1) > 0.5

        if state_cov is None:
            state_cov = torch.full_like(
                new_state_feat[..., 0], self._get_hparam("kalman_p_init", 1.0)
            )
        if kalman_stats is None:
            kalman_stats = {}

        if prev_candidate_state_feat is None:
            delta = torch.ones_like(new_state_feat[..., 0])
        else:
            delta = torch.linalg.norm(new_state_feat - prev_candidate_state_feat, dim=-1)
        delta_mean = delta.mean(dim=1).detach()
        beta_delta = float(self._get_hparam("kalman_ema_beta_delta", 0.05))
        delta_ema = kalman_stats.get("delta_ema", None)
        delta_ema = self._ema_update(delta_ema, delta_mean, beta_delta, batch_update)
        delta_ema = torch.clamp(
            delta_ema,
            min=float(self._get_hparam("kalman_ema_delta_floor", 1e-3)),
        )

        g_t = delta / (delta_ema[:, None] + eps)
        q_min = float(self._get_hparam("kalman_q_min", 1e-2))
        q_max = float(self._get_hparam("kalman_q_max", 3e-1))
        alpha_q = float(self._get_hparam("kalman_alpha_q", 10.0))
        tau_q = float(self._get_hparam("kalman_tau_q", 1.0))
        q_t = q_min + (q_max - q_min) * torch.sigmoid(alpha_q * (g_t - tau_q))

        if fixed_r_t is None:
            fixed_r_t = float(self._get_hparam("kalman_fixed_r", 1.0))
        r_t = torch.full_like(q_t, max(float(fixed_r_t), eps))

        gamma_p = float(self._get_hparam("kalman_gamma_p", 1.0))
        p_pred = gamma_p * state_cov + q_t
        k_t = p_pred / (p_pred + r_t + eps)
        k_t = torch.clamp(
            k_t,
            min=float(self._get_hparam("kalman_k_min", 1e-2)),
            max=float(self._get_hparam("kalman_k_max", 0.99)),
        )

        gain = update_mask * k_t[..., None]
        state_cov_next = (1.0 - k_t) ** 2 * p_pred + (k_t ** 2) * r_t
        state_cov_next = torch.where(batch_update[:, None], state_cov_next, state_cov)
        state_cov_next = torch.clamp(state_cov_next, min=eps)

        kalman_stats["delta_ema"] = delta_ema
        return gain, state_cov_next, kalman_stats

    def _compute_ttt3r_mask(self, update_mask, cross_attn_state):
        if cross_attn_state is None:
            return update_mask
        attn = torch.stack(cross_attn_state, dim=0).mean(dim=(0, 2, 4))
        return update_mask * torch.sigmoid(attn)[..., None]

    def _compute_state_update_gain(
        self,
        update_mask,
        new_state_feat,
        cross_attn_state,
        ...
        if update_type == "filt3r":
            raise RuntimeError(
                f"{update_type} is handled in the recurrent loop to keep kalman_stats state."
            )
        if force_full_update:
            return update_mask, state_cov

        if update_type == "cut3r":
            return update_mask, state_cov
        if update_type == "ttt3r":
            return self._compute_ttt3r_mask(update_mask, cross_attn_state), state_cov
        raise ValueError(
            f"Invalid model type: {self._get_hparam('model_update_type', 'cut3r')}"
        )

    def _advance_prev_buffers(
        self,
        prev_candidate_state_feat,
        prev_feat_i,
        new_state_feat,
        ...
        """
        Advance t-1 buffers only for valid elements (img_mask & update).
        This keeps TAUM/SCUM aligned with consecutive real frames.
        """
        valid = update_mask.squeeze(-1).squeeze(-1) > 0.5
        new_state_detached = new_state_feat.detach()
        new_feat_detached = feat_i.detach()

        if prev_candidate_state_feat is None:
            prev_candidate_state_feat = new_state_detached
        else:
            prev_candidate_state_feat = torch.where(
                valid[:, None, None], new_state_detached, prev_candidate_state_feat
            )

        if prev_feat_i is None:
            prev_feat_i = new_feat_detached
        else:
            prev_feat_i = torch.where(
                valid[:, None, None], new_feat_detached, prev_feat_i
            )
        return prev_candidate_state_feat, prev_feat_i
    ...
    def _forward_impl(self, views, ret_state=False):
        ...
        for i in range(len(views)):
            ...
            new_state_feat, dec, self_attn_state, cross_attn_state, self_attn_img, cross_attn_img = self._recurrent_rollout(
                ...
                reset_mask=views[i]["reset"],
                update=views[i].get("update", None),
                return_attn=need_attn_for_update,
            ) # [1, 768, 768]
            ...
            if update_type == "filt3r":
                if i == 0:
                    ...
                    state_cov = torch.full_like(
                        new_state_feat[..., 0],
                        self._get_hparam("kalman_p_init", 1.5),
                    )
                    kalman_stats = None
                else:
                    update_mask1, state_cov, kalman_stats = (
                        self._compute_kalman_ema_gain_and_cov(
                            update_mask=update_mask,
                            state_cov=state_cov,
                            state_feat=state_feat,
                            new_state_feat=new_state_feat,
                            prev_candidate_state_feat=prev_candidate_state_feat,
                ...
                update_mask1, state_cov = self._compute_state_update_gain(
                    update_mask=update_mask,
                    new_state_feat=new_state_feat,
                    cross_attn_state=cross_attn_state if need_attn_for_update else None,
                    feat_i=feat_i,
                    prev_candidate_state_feat=prev_candidate_state_feat,
                    prev_feat_i=prev_feat_i,
                    state_cov=state_cov,
                    force_full_update=(i == 0),
                    kalman_diag_stats=None,
                )
                state_feat = new_state_feat * update_mask1 + state_feat * (
                    1 - update_mask1
            ...
            state_feat, mem, state_cov, hard_reset_now = self._apply_stream_reset(
                state_feat=state_feat,
                mem=mem,
            ...
            if hard_reset_now:
                ...
                prev_candidate_state_feat, prev_feat_i = self._advance_prev_buffers(
                    prev_candidate_state_feat,
                    prev_feat_i,
                    new_state_feat,
                    feat_i,
        ...
        for i, _view in enumerate(views):
            ...
            if update_type == "filt3r":
                ...
                update_mask1, state_cov = self._compute_state_update_gain(
                    ...
                    prev_feat_i=prev_feat_i,
                    state_cov=state_cov,
                    force_full_update=(i == 0 or prev_reset),
                    kalman_diag_stats=None,
                )
                state_feat = new_state_feat * update_mask1 + state_feat * (
                    1 - update_mask1
