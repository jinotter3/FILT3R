...
def _rope_cast_context(tensor):
    rope_dtype = torch.float16 if tensor.device.type == "cuda" else torch.float32
    if tensor.device.type == "cuda":
        ctx = torch.autocast(device_type="cuda", enabled=False)
    else:
        ctx = contextlib.nullcontext()
    return rope_dtype, ctx
...
class Attention(nn.Module):
    ...
    def forward(self, x, xpos):
        ...
        if self.rope is not None:
            rope_dtype, rope_ctx = _rope_cast_context(q)
            q = q.to(rope_dtype)
            k = k.to(rope_dtype)
            with rope_ctx:
                q = self.rope(q, xpos)
