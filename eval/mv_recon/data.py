...
class NRGBD(BaseStereoViewDataset):
    ...
    @staticmethod
    def _parse_indexed_png(filename, prefix):
    ...
    def _list_frame_ids(self, scene_id, folder, prefix):
        folder_path = osp.join(self.ROOT, scene_id, folder)
        frame_ids = []
        for name in os.listdir(folder_path):
            frame_id = self._parse_indexed_png(name, prefix)
            if frame_id is not None:
                frame_ids.append(frame_id)
        return sorted(frame_ids)

    def get_aligned_frame_ids(self, scene_id, pose_count=None):
        image_ids = self._list_frame_ids(scene_id, "images", "img")
        depth_ids = set(self._list_frame_ids(scene_id, "depth", "depth"))
        aligned_ids = [frame_id for frame_id in image_ids if frame_id in depth_ids]
        if pose_count is not None:
            aligned_ids = [frame_id for frame_id in aligned_ids if frame_id < pose_count]
        return aligned_ids
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            line = self.tuple_list[idx].split(" ")
            scene_id = line[0]
            img_idxs = [int(im_idx) for im_idx in line[1:]]

        else:
            scene_id = self.scene_list[idx // self.num_seq]
            posepath = osp.join(self.ROOT, scene_id, "poses.txt")
            camera_poses, valids = self.load_poses(posepath)
            img_idxs = self.get_aligned_frame_ids(scene_id, pose_count=len(camera_poses))
...
class Long3D(BaseStereoViewDataset):
    ...
    def load_all_scenes(self, base_dir):
        ...
        if self.test_id is None:
            ...
            if isinstance(self.test_id, list):
                requested = set(self.test_id)
            else:
                requested = {self.test_id}
            self.scene_list = [scene for scene in scenes if scene in requested]
    ...
    def _frame_sort_key(frame_name):
        ...
        try:
            return (0, float(frame_stem))
        except ValueError:
            return (1, frame_stem)

    def _load_intrinsics(self, scene_id):
        intrinsics_path = osp.join(
            self.ROOT, scene_id, "images", "scan_images", "camera_intrinsics.txt"
        )
        values = None
        with open(intrinsics_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(tokens) < 4:
                    continue
                values = [float(tokens[0]), float(tokens[1]), float(tokens[2]), float(tokens[3])]
                break
        if values is None:
            raise ValueError(f"Unable to parse camera intrinsics from {intrinsics_path}")
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        frame_names = sorted(frame_names, key=self._frame_sort_key)
        frame_names = frame_names[:: self.kf_every]
        if self.max_frames is not None:
            frame_names = frame_names[: self.max_frames]

        intrinsics_ = self._load_intrinsics(scene_id)
        imgs_idxs = deque(frame_names)
        if self.shuffle_seed >= 0:
            imgs_idxs = shuffle_deque(imgs_idxs, seed=self.shuffle_seed)
        views = []

        while len(imgs_idxs) > 0:
            im_name = imgs_idxs.popleft()
            impath = osp.join(image_dir, im_name)
            rgb_image = imread_cv2(impath)
            h, w = rgb_image.shape[:2]

            # Long3D only ships RGB + global point cloud; provide a valid placeholder depth
            # so the existing mv_recon data pipeline can run in inference mode.
            ...
            frame_id = osp.splitext(im_name)[0]
            views.append(
                dict(
                    img=rgb_image,
                    depthmap=depthmap,
                    ...
                    camera_intrinsics=intrinsics,
                    dataset="long3d",
                    label=osp.join(scene_id, frame_id),
                    instance=impath,
                )
            )
