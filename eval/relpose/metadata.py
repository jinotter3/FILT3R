...
CONTEXT_MEMORY_FRAMES = os.environ.get(
    "CONTEXT_MEMORY_FRAMES",
    "data/context_memory/frames",
)
CONTEXT_MEMORY_JSONS = os.environ.get(
    "CONTEXT_MEMORY_JSONS",
    "data/context_memory/jsons",
)
...
dataset_metadata = {
    ...
    "context_memory": {
        "img_path": CONTEXT_MEMORY_FRAMES,
        "anno_path": CONTEXT_MEMORY_JSONS,
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq),
        "gt_traj_func": lambda img_path, anno_path, seq: os.path.join(
            anno_path, f"{seq}.json"
        ),
        "traj_format": "context_memory",
        "seq_list": None,
        "full_seq": True,
        "mask_path_seq_func": lambda mask_path, seq: None,
        "skip_condition": None,
...
scannet_s1_numbers = [100, 300, 500, 700, 900, 1100, 1300, 1500]
scannet_s1_configs = {
    f"scannet_s1_{num}": {
        "img_path": "data/long_scannet_s1",
        "mask_path": None,
        "dir_path_func": lambda img_path, seq, num=num: os.path.join(img_path, seq, f"color_{num}"),
        "gt_traj_func": lambda img_path, anno_path, seq, num=num: os.path.join(
            img_path, seq, f"pose_{num}.txt"
    ...
    for num in scannet_s1_numbers
}
dataset_metadata.update(scannet_s1_configs)
...
def process_scannet(args, img_path):
    metadata = dataset_metadata.get(args.eval_dataset, dataset_metadata["scannet"])
    dir_path_func = metadata["dir_path_func"]
    seq_list = sorted(glob.glob(f"{img_path}/*"))
    for seq_dir in tqdm(seq_list):
        seq_name = os.path.basename(seq_dir)
        dir_path = dir_path_func(img_path, seq_name)
        filelist = sorted(glob.glob(f"{dir_path}/*.jpg"))
        save_dir = f"{args.output_dir}/{seq_name}"
        yield filelist, save_dir
